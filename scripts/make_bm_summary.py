...
def make_summary(experiments_root="best_model_experiment/experiments_v1", summary_path="best_model_experiment/experiments_v1/summary.csv"):
    ...
    for sweep_key in os.listdir(experiments_root):
        ...
        for sweep_value in os.listdir(sweep_key_path):
            ...
            for seed_dir in os.listdir(sweep_value_path):
                ...
                row = {
                    "run_id": f"{sweep_key}_{sweep_value}_{seed_dir}",
                    "sweep_key": sweep_key,
                    "sweep_value": sweep_value,
                    "feature_map": config.get("feature_map", ""),
                    "ansatz": config.get("ansatz", ""),
                    "ansatz_depth": config.get("ansatz_depth", ""),
